...
export default function Nav({
  ...
  const renderLink = ({ sub, role, ...rest }: SideLink) => {
    ...
    const rolle = useSelector((state: any) => state.auth.role);

    if (role && role.toLowerCase() !== rolle.toLowerCase()) {
      return null;
    }
    if (isCollapsed && sub)
      return (
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        "h-10 justify-start text-wrap rounded-md px-6",
        subLink && "rounded-none h-10 w-full border-l border-l-slate-500 px-2"
      )}
    ...
    <Collapsible defaultOpen={isChildActive}>
      <CollapsibleTrigger
        className={cn(
          ...
          "group h-10 w-full justify-start rounded-xl px-6"
        )}
      >
        <div className="mr-2">{icon}</div>
        {title}
